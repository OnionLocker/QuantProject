...
TIMEFRAME = '1h'         
# ⚠️ 变更：弃用 TRADE_PERCENTAGE，改为 RISK_PER_TRADE_PCT (例如 0.01 代表单笔最大亏本金 1%)
...
my_risk_manager = RiskManager(max_trade_amount=1000, is_trading_allowed=True)
...
def run_bot():
    ...
    if pos["status"] == "both":
        ...
        if state["position_amount"] > 0:
            msg = "🚨 对账失败：本地有持仓，交易所为空！请人工删除 trade_state.json 后重启。"
            bot_logger.error(msg); send_telegram_msg(msg); raise RuntimeError(msg)
        else:
            cancel_all_algo_orders(SYMBOL)
    ...
    while True:
        try:
            ...
            if df is not None:
                ...
                status_text = "空仓观望中" if state["position_amount"] == 0 else f"持有 {state['position_side']} 单 ({state['position_amount']}张)"
                ...
                if state["position_amount"] == 0:
                    if action in ["BUY", "SELL"]:
                        ...
                        if dynamic_trade_amount < 1:
                            bot_logger.info("❌ 计算出的开仓数量小于 1 张，可能止损空间太大或余额不足，放弃该信号！")
                            action = "HOLD"
                    ...
                    if action in ["BUY", "SELL"]:
                        ...
                        if my_risk_manager.check_order(SYMBOL, open_side, dynamic_trade_amount):
                            order = place_market_order(SYMBOL, open_side, dynamic_trade_amount, reduce_only=False, pos_side=pos_side)

                            if order:
                                fill_price = float(order.get("average") or order.get("price") or current_price)
                                
                                sl_order = place_algo_order(SYMBOL, close_side, dynamic_trade_amount, target_sl, pos_side, "sl")
                                tp_order = place_algo_order(SYMBOL, close_side, dynamic_trade_amount, target_tp, pos_side, "tp")
                                
                                if not sl_order or not tp_order:
                                    bot_logger.error("🚨 保护单挂载失败！回滚强平！")
                                    send_telegram_msg("🚨 <b>紧急避险机制触发</b>\n正在强平保命，强制关闭机器人！")
                                    cancel_all_algo_orders(SYMBOL)
                                    place_market_order(SYMBOL, close_side, dynamic_trade_amount, reduce_only=True, pos_side=pos_side)
                                    import sys
                ...
                # ================= 场景 2: 平多 =================
                    ...
                    if sell_reason and my_risk_manager.check_order(SYMBOL, 'sell', state["position_amount"]):
                        cancel_all_algo_orders(SYMBOL)
                        order = place_market_order(SYMBOL, "sell", state["position_amount"], reduce_only=True, pos_side="long")
                        if order:
                            fill_price = float(order.get("average") or order.get("price") or current_price)
                            _log_close_trade("🎉", "平多单", sell_reason, fill_price, state)
                            clear_state()
                            state = load_state()
                ...
                # ================= 场景 3: 平空 =================
                    ...
                    if sell_reason and my_risk_manager.check_order(SYMBOL, 'buy', state["position_amount"]):
                        cancel_all_algo_orders(SYMBOL)
                        order = place_market_order(SYMBOL, "buy", state["position_amount"], reduce_only=True, pos_side="short")
                        if order:
                            fill_price = float(order.get("average") or order.get("price") or current_price)
                            _log_close_trade("🎉", "平空单", sell_reason, fill_price, state)
                            clear_state()
                            state = load_state()
...
def _log_close_trade(emoji, action_name, reason, current_price, state):
    ...
    # 修正：通过包含负号的判断计算正确的多空方向利润
    is_long = "多" in action_name
    gross_profit = (current_price - state["entry_price"]) * state["position_amount"] * CONTRACT_SIZE if is_long else (state["entry_price"] - current_price) * state["position_amount"] * CONTRACT_SIZE
    net_profit = gross_profit - total_fees
    net_roi = (net_profit / state["margin_used"]) * 100
    
    tg_msg = (f"{emoji if net_profit > 0 else '🩸'} <b>{action_name}</b>\n价格: {current_price:.2f}\n"
              f"净盈亏: {net_profit:+.2f} U\nROI: {net_roi:+.2f}%\n原因: {reason}")
    send_telegram_msg(tg_msg)

    # --- 【新增】把平仓和算好的净利润(net_profit)精准记录到 SQLite ---
